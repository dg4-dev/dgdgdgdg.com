...
(function () {
  ...
  function InfiniteCarousel(element, direction, numVisible, options) {
    ...
    this.transitionEndEvent = Utils.getVendorPrefix(
      CSS_VENDOR_PREFIX.TRANSITION_END
    );
   ...
   * @api private
   */
  InfiniteCarousel.prototype._setContainerAttributes = function (
    position,
    style
  ) {
    ...
    var offset =
      this.direction === "vertical"
        ? this.firstItem.offsetHeight
        : this.firstItem.offsetWidth;
    this.scrollingDistance =
      offset +
      parseInt(getComputedStyle(this.firstItem)[style].slice(0, -2), 10);
    ...
    if (this.direction === "horizontal") {
      this.container.setAttribute(
        "style",
        "width: " +
          this.scrollingDistance * (this.numItems + this.numVisible) +
          "px"
      );
    }
  };
  ...
  InfiniteCarousel.prototype._animateContainer = function () {
    ...
    var translate =
      this.direction === "vertical"
        ? "translate3d(0, " + this.pos + "px, 0)"
        : "translate3d(" + this.pos + "px, 0, 0)";
...
var infinitecarousel = new InfiniteCarousel("#meanItems", "horizontal", 9, {
  timerDuration: 0,
